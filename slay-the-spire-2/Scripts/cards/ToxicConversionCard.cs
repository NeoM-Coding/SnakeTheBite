...
public class ToxicConversionCard : SnakeTheBiteCardModel
{
    ...
    protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
        ...
        if (poisonAmount > 0)
        {
            ...
            {
                await PowerCmd.Apply<PoisonPower>(new ThrowingPlayerChoiceContext(), enemy, poisonAmount * 2, Owner.Creature, this, false);
            }
        }
        ...
        if (IsUpgraded)
        {
            ...
            if (truePoisonAmount > 0)
            {
                foreach (var enemy in enemies)
                {
                    await PowerCmd.Apply<TruePoisonPower>(new ThrowingPlayerChoiceContext(), enemy, truePoisonAmount * 2, Owner.Creature, this, false);
                }
            }
        }
    }
